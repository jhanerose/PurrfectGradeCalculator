...
let count = 50;
for(var i = 0; i<50; i++){
    let leftSparkle = Math.floor(Math.random() * container.clientWidth);
    let topSparkle = Math.floor(Math.random() * container.clientHeight);
    let widthSparkle = Math.floor(Math.random() * 50);
    let timeSparkle = Math.floor((Math.random() * 5) + 5);
    let blurSparkle = Math.floor(Math.random() * 10);
    console.log(leftSparkle);
    let div = document.createElement('div');
    div.classList.add('sparkle');
    div.style.left = leftSparkle + 'px';
    div.style.top = topSparkle + 'px';
    div.style.width = widthSparkle + 'px';
    div.style.height = widthSparkle + 'px';
    div.style.animationDuration = timeSparkle + 's';
    div.style.filter = "blur(" + blurSparkle + "px)";
